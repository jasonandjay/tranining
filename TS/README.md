...
tsc --init
```
工程化TS配置项，引入tsconfig.json配置文件
[Tsconfig的配置](https://www.typescriptlang.org/v2/en/tsconfig)
...
### 索引类型与映射类型
- keyof 索引类型操作符，来获取泛型所有public属性名构成联合类型
```js
class Images {
...
- 解析
- 转换 （ts的api, ts-loader配合webpack, 自定义编译器包装器）
- 生成